...
class App {
  ...
  init() {
    try{
    this.$monaco = monaco.editor.create(this.$leftPane, {
      value: 'console.log("Hello, Vite and Monaco Editor!");',
      language: 'css',
      theme: 'vs-dark', // You can change the theme if you prefer
    });
  }catch(r){}
    this.$textFilter.value = this.browserList.join(", ");
    // this.$leftPane.innerHTML = CSS_EXAMPLE;
    this.$monaco.setValue(CSS_EXAMPLE);
    // this.$leftPane.focus();
  ...
  window.monaco = this.$monaco;
    ...
    this.runPrefixer();
    this.addVersion();
    this.$leftPane.addEventListener("resize", function () {
      this.$monaco.layout();
    });
    window.onresize = function(){
      window.monaco.layout();
    };
  }

  addVersion() {
    this.$version.innerHTML = `Postcss: <b>v${postcssVersion}</b>, FLOOD: <b>v${autoprefixerVersion}</b>`;
  }
  ...
  listeners() {
    ...
    this.$monaco.onDidChangeModelContent(this.runPrefixer.bind(this));
  ...
  async runPrefixer() {
    const inputCSS = this.$monaco.getValue();// //this.$leftPane.value;
    const params = {
